...
/**
 * This test tests the class BitmapAdapter.
 *
...
class BitmapAdapterTest {
    ...
    void bitmapConversionTest(BufferedImage image) throws IOException {
        byte[] output = bitmapAdapter.convert(image);

        try (ByteArrayInputStream byteArrayInputStream = new ByteArrayInputStream(output)) {
            ImageInputStream imageInputStream = ImageIO.createImageInputStream(byteArrayInputStream);
            assertEquals("bmp", ImageIO.getImageReaders(imageInputStream).next().getFormatName());
        }
    }
